...
SET Sysprepxml=%~1
ECHO %DATE% %TIME%[Log TRACE]  C:\OEM\preload\utility\GCM\MRDCreaterX_%CPUArchitecture%.exe /FV /IMWhiteListPath:c:\OEM\preload\DPOP\GCMReadiness\IM_WhiteList.txt >>%LogPath%
C:\OEM\preload\utility\GCM\MRDCreaterX_%CPUArchitecture%.exe /FV /IMWhiteListPath:c:\OEM\preload\DPOP\GCMReadiness\IM_WhiteList.txt >>%LogPath% 2>&1
ECHO.>>%LogPath%
IF /I EXIST C:\OEM\preload\utility\GCM\FVMRD.xml (
	ECHO !DATE! !TIME![Log TRACE]  call _MergeNewGCMMRDandSysprepXML.vbs to update Notification Area. >>%LogPath%
	ECHO !DATE! !TIME![Log TRACE]  Cscript.exe /nologo _MergeNewGCMMRDandSysprepXML.vbs specialize Microsoft-Windows-MicrosoftEdgeBrowser FavoriteBarItems C:\OEM\preload\utility\GCM\FVMRD.xml "%Sysprepxml%" >> %LogPath% 2>&1
	ECHO.>>%LogPath%
	ECHO !DATE! !TIME![Log TRACE]  ^|----------------------------- Start -----------------------------------^| >>%LogPath%
	Cscript.exe /nologo _MergeNewGCMMRDandSysprepXML.vbs specialize Microsoft-Windows-MicrosoftEdgeBrowser FavoriteBarItems C:\OEM\preload\utility\GCM\FVMRD.xml "%Sysprepxml%" >> %LogPath% 2>&1
