...
echo %DATE% %TIME% [Log TRACE] This is Windows10 20H1H2, check OA3
echo %DATE% %TIME% [Log TRACE] This is Windows10 20H1H2, check OA3 >> %LogFile%
...
find /i "CPU=" "C:\OEM\Preload\Command\PAP\PAP*.ini" > C:\OEM\FIVT\ModularizationItems\CheckProductKey\Tools\W1020H1H2\CPUInfo.txt
for /f "tokens=2 delims==" %%f in (C:\OEM\FIVT\ModularizationItems\CheckProductKey\Tools\W1020H1H2\CPUInfo.txt) do (
    set CPU_Type=%%f
    echo CPU Type=[!CPU_Type!]
    echo CPU Type=[!CPU_Type!] >> %LogFile%
)

echo %DATE% %TIME% [Log TRACE] "%~dp0"oa3tool\!CPU_Type!\oa3tool.exe /Validate
echo %DATE% %TIME% [Log TRACE] "%~dp0"oa3tool\!CPU_Type!\oa3tool.exe /Validate >> %LogFile%
oa3tool\!CPU_Type!\oa3tool.exe /Validate > oa3tool_result.log
if %errorlevel% equ 0 (
...
rem 20201215 add by Kevin Chang request
CheckOA3_%processor_architecture%.dat > oa3dat_result.log
if %errorlevel% equ 0 (
	echo !DATE! !TIME! [Log TRACE] Return code is [%errorlevel%], Check OEM MSDM pass
	echo !DATE! !TIME! [Log TRACE] Return code is [%errorlevel%], Check OEM MSDM pass >> %LogFile%
) else (
	echo !DATE! !TIME! [Log ERROR] Fail, return code is [%errorlevel%], Check OEM MSDM fail
	echo !DATE! !TIME! [Log ERROR] Fail, return code is [%errorlevel%], Check OEM MSDM fail >> %LogFile%
	echo !DATE! !TIME! [Log ERROR] Fail, return code is [%errorlevel%], Check OEM MSDM fail > Fail.tag
	oa3tool\!CPU_Type!\oa3tool.exe /CheckEdition /Online >> oa3tool_result.log
	type oa3tool_result.log >> Fail.tag
	goto END
)
...
echo %DATE% %TIME% [Log TRACE] "%~dp0"oa3tool\!CPU_Type!\oa3tool.exe /CheckEdition /Online
echo %DATE% %TIME% [Log TRACE] "%~dp0"oa3tool\!CPU_Type!\oa3tool.exe /CheckEdition /Online >> %LogFile%
oa3tool\!CPU_Type!\oa3tool.exe /CheckEdition /Online >> oa3tool_result.log
if %errorlevel% equ 0 (
