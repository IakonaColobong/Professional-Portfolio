...
echo %DATE% %TIME% [Log TRACE] This is Windows10 19H2, check OA3
echo %DATE% %TIME% [Log TRACE] This is Windows10 19H2, check OA3 >> %LogFile%
...
if exist "C:\OEM\Preload\Command\POP?????6R*.ini" (
    echo !DATE! !TIME! [Log TRACE] POP is Win10 Pro Education >> %LogFile%
    echo !DATE! !TIME! [Log TRACE] Need check OS currect edition >> %LogFile%
    echo !DATE! !TIME! [Log TRACE] Dism.exe /Online /Get-CurrentEdition /English >> %LogFile%
    Dism.exe /Online /Get-CurrentEdition /English >> %LogFile%
    echo !DATE! !TIME! [Log TRACE] Dism.exe /Online /Get-CurrentEdition /English ^| find /i "Current Edition : Education" >> %LogFile%
    Dism.exe /Online /Get-CurrentEdition /English | find /i "Current Edition : Education"
    set FindEducation=!errorlevel!
    echo !DATE! !TIME! [Log TRACE] errorlevel is !FindEducation! >>%LogFile%
    if !FindEducation! EQU 0 (
        echo !DATE! !TIME! [Log TRACE] Current Edition : Education
        echo !DATE! !TIME! [Log TRACE] Current Edition : Education >>%LogFile%
        echo !DATE! !TIME! [Log TRACE] Skip CheckProductkey of FIVT
        echo !DATE! !TIME! [Log TRACE] Skip CheckProductkey of FIVT >>%LogFile%
        echo !DATE! !TIME! [Log TRACE] Pass, Current Edition : Education, Skip Productkeycheck  > Pass.tag
        goto END
    ) else (
        echo !DATE! !TIME! [Log TRACE] Current Edition is not Win10 Education
        echo !DATE! !TIME! [Log TRACE] Current Edition is not Win10 Education >>%LogFile%      
    )
) else (
    echo !DATE! !TIME! [Log TRACE] POP is not Win10 Pro Education
    echo !DATE! !TIME! [Log TRACE] POP is not Win10 Pro Education >> %LogFile%   
)
