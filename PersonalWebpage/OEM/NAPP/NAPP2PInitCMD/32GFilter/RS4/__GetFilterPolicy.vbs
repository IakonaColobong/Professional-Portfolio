...
IF (arg_c=1) Then
	...
	Set PromNodes=xmlDoc.selectNodes("//SCL/Programs/Program[contains(@Product,'32G App filter_RS4')]")
	...
	IF PromNodes.Length > 0 Then
		For i = 0 To PromNodes.Length - 1
			...
			Wscript.Echo "32G App filter_RS4 is [" & pid & "]"
			path = ".\32GAppFilterRS4_Found.tag"
			set objFile = fso.CreateTextFile(path,True)
			objFile.Write "32G App filter_RS4 is [" & pid & "]" & vbCrLf
			objFile.Close
		Next
	Else
		Wscript.Echo "Can not find [32G App filter_RS4]. leave"
	End IF
	Wscript.Echo
Else
	WScript.Echo arg_c
